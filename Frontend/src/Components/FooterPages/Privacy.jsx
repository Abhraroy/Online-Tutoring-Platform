import React from 'react';

const Privacy = () => {
  return (
    <div className="w-full min-h-[60vh] flex items-center justify-center bg-gray-50 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl w-full bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Privacy Policy</h1>
        <p className="text-gray-600 leading-relaxed text-sm sm:text-base mb-3">
          This is a static privacy policy page for TutorConnect. It is meant to describe how user data
          would be handled in a real production application.
        </p>
        <p className="text-gray-600 leading-relaxed text-sm sm:text-base">
          In this demo, no real personal data is shared outside your environment. All information, such as
          student and tutor profiles, exists only within this sample project.
        </p>
      </div>
    </div>
  );
};

export default Privacy;


