  ...
  useEffect(()=>{
    const fetchBookedSessions = async ()=>{
      try{
        ...
        setBookedSessionsCount(response.data.bookedSessions?.length || 0);
      }catch(error){
        // Handle 404 gracefully - just means no booked sessions
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8'>
          ...
          <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-200'>
            <div className='flex items-center'>
              ...
              <div className='ml-4'>
                <p className='text-sm font-medium text-gray-600'>Booked Sessions</p>
                <p className='text-2xl font-bold text-gray-900'>{bookedSessionsCount}</p>
              </div>
            </div>
          </div>
        </div>
